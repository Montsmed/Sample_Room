                ...
                if st.button(f"Select {layer_label}", key=f"btn_{layer_label}"):
                    ...
                    if prev_layer and prev_layer != layer_label:
                        ...
                        if prev_key in st.session_state:
                            ...
                            # Update temp memory
                            st.session_state[prev_key] = prev_edits.copy()
...
if selected_layer:
    ...
    with col_del:
        delete_rows = st.button("Delete Selected Rows", key=f"del_{selected_layer}")
    ...
    # Handle row deletion
    if delete_rows and grid_response['selected_rows']:
        # Remove selected rows by index
        selected = pd.DataFrame(grid_response['selected_rows'])
        # To avoid index mismatch, use a merge with an indicator
        merged = edited_data.merge(selected, how='outer', indicator=True)
        edited_data = merged[merged['_merge'] == 'left_only'].drop(columns=['_merge'])
    ...
    if st.button("Save Changes", key=f"save_{selected_layer}"):
        ...
        # Update main data and temp memory
